...
{
  source('~/Simulation_BiRS/Seeded_Detect.R')
  source('~/Simulation_BiRS/Wild_Detect.R')
  
  print(paste0(' mulist', l, ' :start'))
  
  p = 8192; n = 600; m = 400; nsimu = 100
  MB = 1000; alpha = 0.05; Lmin = 128; Lmax = 320  
  ...
  SimuL = function(i)
  {
    ...
    aaWBS = Sys.time()
    reWBS = WildDetect(Xi, Yi, M = 10000, m = 32, MB, alpha) 
    bbWBS = Sys.time()
    DifTWBS = bbWBS - aaWBS
    
    aaSBS = Sys.time()
    reSBS = SeedDetect(Xi, Yi, m = 32, MB, alpha, decay = 2)
    bbSBS = Sys.time()
    DifTSBS = bbSBS - aaSBS
    
    return(list(reBSD = reBSD, reWBS = reWBS, reSBS = reSBS, reSCQ = reSCQ, DifTBSD = DifTBSD, DifTWBS = DifTWBS, DifTSBS = DifTSBS, DifTSCQ = DifTSCQ))
  }
  ...
  save(list = c('res', 'mu'), file = paste0('Review-Scan-Cpts/WNS-Mulist', l, '.RData'))
