rm(list = ls())

load('Review-BiRS-cpts/MES-BSD_Describe.RData')
load('Review-BiRS-cpts/MES-BSB_Describe.RData')
load('Review-BiRS-cpts/MES-WBB_Describe.RData')
load('Review-BiRS-cpts/MES-SBB_Describe.RData')

powerBSD = matrix(powerBSD/100, 5, 4)
powerBSB = matrix(powerBSB/100, 5, 4)
powerWBB = matrix(powerWBB/100, 5, 4)
powerSBB = matrix(powerSBB/100, 5, 4)

TRBSD = matrix(apply(TrateBSD, 2, mean), 5, 4)
TRBSB = matrix(apply(TrateBSB, 2, mean), 5, 4)
TRWBB = matrix(apply(TrateWBB, 2, mean), 5, 4)
TRSBB = matrix(apply(TrateSBB, 2, mean), 5, 4)

FRBSD = matrix(apply(FrateBSD, 2, mean), 5, 4)
FRBSB = matrix(apply(FrateBSB, 2, mean), 5, 4)
FRWBB = matrix(apply(FrateWBB, 2, mean), 5, 4)
FRSBB = matrix(apply(FrateSBB, 2, mean), 5, 4)


delta = c(0.20, 0.25, 0.30, 0.35, 0.40)
for (j in 1:4)
{
  nameTj = paste0('Figures/MES-TPR-BSB_beta', j, '.jpg')
  nameFj = paste0('Figures/MES-FDR-BSB_beta', j, '.jpg')
  
  # nameCTj = paste0('Review-Figures/MES-CP-TPR_beta', j, '.jpg')
  # nameCFj = paste0('Review-Figures/MES-CP-FDR_beta', j, '.jpg')
  
  mainTj = paste0('MES, beta = ', j, ' TPR')
  mainFj = paste0('MES, beta = ', j, ' FDR')
  
  jpeg(filename = nameTj, width = 1200, height = 900, quality = 100)
  plot(delta, TRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainTj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, TRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("right", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
  
  jpeg(filename = nameFj, width = 1200, height = 900, quality = 100)
  plot(delta, FRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainFj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, FRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("topleft", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
}

#############################################################################################################################################################################################

rm(list = ls())

load('Review-BiRS-cpts/MNS-BSD_Describe.RData')
load('Review-BiRS-cpts/MNS-BSB_Describe.RData')
load('Review-BiRS-cpts/MNS-WBB_Describe.RData')
load('Review-BiRS-cpts/MNS-SBB_Describe.RData')

powerBSD = matrix(powerBSD/100, 5, 4)
powerBSB = matrix(powerBSB/100, 5, 4)
powerWBB = matrix(powerWBB/100, 5, 4)
powerSBB = matrix(powerSBB/100, 5, 4)

TRBSD = matrix(apply(TrateBSD, 2, mean), 5, 4)
TRBSB = matrix(apply(TrateBSB, 2, mean), 5, 4)
TRWBB = matrix(apply(TrateWBB, 2, mean), 5, 4)
TRSBB = matrix(apply(TrateSBB, 2, mean), 5, 4)

FRBSD = matrix(apply(FrateBSD, 2, mean), 5, 4)
FRBSB = matrix(apply(FrateBSB, 2, mean), 5, 4)
FRWBB = matrix(apply(FrateWBB, 2, mean), 5, 4)
FRSBB = matrix(apply(FrateSBB, 2, mean), 5, 4)

delta = c(0.20, 0.25, 0.30, 0.35, 0.40)
for (j in 1:4)
{
  nameTj = paste0('Figures/MNS-TPR-BSB_beta', j, '.jpg')
  nameFj = paste0('Figures/MNS-FDR-BSB_beta', j, '.jpg')
  
  # nameCTj = paste0('Review-Figures/MES-CP-TPR_beta', j, '.jpg')
  # nameCFj = paste0('Review-Figures/MES-CP-FDR_beta', j, '.jpg')
  
  mainTj = paste0('MNS, beta = ', j, ' TPR')
  mainFj = paste0('MNS, beta = ', j, ' FDR')
  
  jpeg(filename = nameTj, width = 1200, height = 900, quality = 100)
  plot(delta, TRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainTj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, TRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("right", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
  
  jpeg(filename = nameFj, width = 1200, height = 900, quality = 100)
  plot(delta, FRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainFj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, FRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("topleft", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
  
}


#############################################################################################################################################################################################

rm(list = ls())

load('Review-BiRS-cpts/WES-BSD_Describe.RData')
load('Review-BiRS-cpts/WES-BSB_Describe.RData')
load('Review-BiRS-cpts/WES-WBB_Describe.RData')
load('Review-BiRS-cpts/WES-SBB_Describe.RData')

powerBSD = matrix(powerBSD/100, 5, 4)
powerBSB = matrix(powerBSB/100, 5, 4)
powerWBB = matrix(powerWBB/100, 5, 4)
powerSBB = matrix(powerSBB/100, 5, 4)

TRBSD = matrix(apply(TrateBSD, 2, mean), 5, 4)
TRBSB = matrix(apply(TrateBSB, 2, mean), 5, 4)
TRWBB = matrix(apply(TrateWBB, 2, mean), 5, 4)
TRSBB = matrix(apply(TrateSBB, 2, mean), 5, 4)

FRBSD = matrix(apply(FrateBSD, 2, mean), 5, 4)
FRBSB = matrix(apply(FrateBSB, 2, mean), 5, 4)
FRWBB = matrix(apply(FrateWBB, 2, mean), 5, 4)
FRSBB = matrix(apply(FrateSBB, 2, mean), 5, 4)

delta = c(0.20, 0.25, 0.30, 0.35, 0.40)
for (j in 1:4)
{
  nameTj = paste0('Figures/WES-TPR-BSB_beta', j, '.jpg')
  nameFj = paste0('Figures/WES-FDR-BSB_beta', j, '.jpg')
  
  # nameCTj = paste0('Review-Figures/MES-CP-TPR_beta', j, '.jpg')
  # nameCFj = paste0('Review-Figures/MES-CP-FDR_beta', j, '.jpg')
  
  mainTj = paste0('WES, beta = ', j, ' TPR')
  mainFj = paste0('WES, beta = ', j, ' FDR')
  
  jpeg(filename = nameTj, width = 1200, height = 900, quality = 100)
  plot(delta, TRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainTj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, TRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("right", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
  
  jpeg(filename = nameFj, width = 1200, height = 900, quality = 100)
  plot(delta, FRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainFj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, FRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("topleft", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
}

#############################################################################################################################################################################################

rm(list = ls())

load('Review-BiRS-cpts/WNS-BSD_Describe.RData')
load('Review-BiRS-cpts/WNS-BSB_Describe.RData')
load('Review-BiRS-cpts/WNS-WBB_Describe.RData')
load('Review-BiRS-cpts/WNS-SBB_Describe.RData')

powerBSD = matrix(powerBSD/100, 5, 4)
powerBSB = matrix(powerBSB/100, 5, 4)
powerWBB = matrix(powerWBB/100, 5, 4)
powerSBB = matrix(powerSBB/100, 5, 4)

TRBSD = matrix(apply(TrateBSD, 2, mean), 5, 4)
TRBSB = matrix(apply(TrateBSB, 2, mean), 5, 4)
TRWBB = matrix(apply(TrateWBB, 2, mean), 5, 4)
TRSBB = matrix(apply(TrateSBB, 2, mean), 5, 4)

FRBSD = matrix(apply(FrateBSD, 2, mean), 5, 4)
FRBSB = matrix(apply(FrateBSB, 2, mean), 5, 4)
FRWBB = matrix(apply(FrateWBB, 2, mean), 5, 4)
FRSBB = matrix(apply(FrateSBB, 2, mean), 5, 4)

delta = c(0.20, 0.25, 0.30, 0.35, 0.40)
for (j in 1:4)
{
  nameTj = paste0('Figures/WNS-TPR-BSB_beta', j, '.jpg')
  nameFj = paste0('Figures/WNS-FDR-BSB_beta', j, '.jpg')
  
  # nameCTj = paste0('Review-Figures/MES-CP-TPR_beta', j, '.jpg')
  # nameCFj = paste0('Review-Figures/MES-CP-FDR_beta', j, '.jpg')
  
  mainTj = paste0('WNS, beta = ', j, ' TPR')
  mainFj = paste0('WNS, beta = ', j, ' FDR')
  
  jpeg(filename = nameTj, width = 1200, height = 900, quality = 100)
  plot(delta, TRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainTj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, TRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("right", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
  
  jpeg(filename = nameFj, width = 1200, height = 900, quality = 100)
  plot(delta, FRBSD[ ,j], type = 'b', pch = 1, lty = 1, lwd = 4, col = 1, main = mainFj, 
       ylab = "", xlab = "delta", cex.lab = 3, cex.axis = 2.8, ylim = c(0, 1), cex.main = 4)
  lines(delta, FRBSB[ ,j], type = "b", pch = 2, lty = 1, col = 2, lwd = 4)
  legend("topleft", c("BSD", "BSB"), pch = c(1, 2, 3), lty = 1, box.col = "grey", cex = 3.5, col = c(1, 2, 3), lwd = 4)
  dev.off()
}


#############################################################################################################################################################################################