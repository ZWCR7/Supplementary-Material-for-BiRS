...
for (j in 7:7)
{
  ...
  SimuL = function(s)
  {
    ...
    for (hh in 1:10)
    {
      ...
      aaSCD = Sys.time()
      reSCD = SCAN_DCF(Xi, Yi, Lmin, Lmax, skip = 1, MB, alpha)
      bbSCD = Sys.time()
      
      diffSCD = bbSCD - aaSCD
      save(list = c('reSCD', 'diffSCD'), file = paste0('Genetic_RES/SCAN-DCF_delta = ', deltav[j], '_hh = ', hh, '_seed = ', s, '.RData'))
      
      #############################################################################
      
      aaDCF = Sys.time()
      reDCF = XFTest(Xi, Yi, MB, alpha)
      bbDCF = Sys.time()

      diffDCF = bbDCF - aaDCF
      save(list = c('reDCF', 'diffDCF'), file = paste0('Genetic_RES/DCF_delta = ', deltav[j], '_hh = ', hh, '_seed = ', s, '.RData'))
